...
from assess.orm.models import Customer
import os
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # search to see if access code is valid
            code = request.POST.get('access_code', '')
            try:
                customer = Customer.objects.get(access_code=code)
    ...
    for key, value in extra_context.items():
        context[key] = callable(value) and value() or value
    if valid_access_code == False:
        context['error'] = 'A valid access code is required.'
